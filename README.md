...
## 6. Database Security & Multi-Tenancy

Modern B2B SaaS applications are fundamentally multi-tenant. A senior engineer must design the database to prevent cross-tenant data leakage at the lowest possible layer.

### Key Concepts to Master

- **Row-Level Security (RLS)**:
    - Enforcing tenant isolation directly within the PostgreSQL engine.
    - Using `SET LOCAL` and SQLAlchemy `execution_options` to pass tenant context securely without leaking connections.
- **Roles & Privileges (RBAC)**:
    - The Principle of Least Privilege: Never connect the application as the `postgres` superuser.
    - Setting up `app_readonly`, `app_readwrite`, and migration-specific roles.
    - Understanding `DEFAULT PRIVILEGES` so new tables automatically inherit the correct permissions.

## 7. Modern Extensions & Advanced Workloads

PostgreSQL is the ultimate Swiss Army Knife. A senior engineer leverages native extensions before reaching for external microservices (like Elasticsearch or Pinecone).

### Key Concepts to Master

- **Native Full-Text Search (FTS)**:
    - Understanding `tsvector` (documents) and `tsquery` (search terms).
    - Implementing GIN indexes for lightning-fast keyword matching, ranking, and highlighting.
- **pgvector & AI/ML Workloads**:
    - Storing high-dimensional floating-point arrays generated by LLMs.
    - Performing Nearest Neighbor (KNN) and Approximate Nearest Neighbor (ANN) similarity searches using HNSW or ivfflat indexes.
